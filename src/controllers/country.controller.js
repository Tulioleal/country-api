    ...
    async getCountry(req, res) {
        ...
        try {
            const countryResponse = await fetch(
                `${process.env.DATE_NAGGER_ROUTE}/CountryInfo/${countryCode}`
            )
            ...
            return res
                .status(500)
                .json({
                    error: 'An error occurred while fetching country info',
                    details: error,
                })
        }
        ...
        try {
            const populationResponse = await fetch(
                `${process.env.COUNTRIES_NOW}/countries/population`,
                {
                    method: 'POST',
                    headers: {
        ...
        try {
            const flagResponse = await fetch(
                `${process.env.COUNTRIES_NOW}/countries/flag/images`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
            ...
            return res
                .status(500)
                .json({
                    error: 'An error occurred while fetching flag data',
                    details: error,
                })
        }
        ...
        try {
            res.json({
                ...
                populationByYear: population.data
